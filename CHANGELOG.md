...
### Added
- `voice_settings` on the ElevenLabs-compatible endpoint is now validated and its `speed` is honoured; `stability`, `similarity_boost`, `style` and `use_speaker_boost` stay accepted-and-ignored, and an out-of-range value returns 422 instead of being silently clamped (#7)
...
- `POST /v1/chat/completions` and `GET /v1/models` — the chat half of the OpenAI-compatible API, relayed to whichever LLM the install already uses, so any tool with a "custom OpenAI provider" setting can share one backend for speech and text (#1652)
- The Agents workspace now shows conversation state as sound: a bar visualizer driven by the live session, so listening, thinking, speaking and barge-in are legible at a glance (#1652)
...
- `docs/agentic-voice.md` now covers the chat relay and how to point WhispVoice's AI enhancement at it — including the honest part, that WhispVoice's transcription is on-device only and cannot be aimed anywhere (#1652)
