/*
 * Vendored from ElevenLabs UI (https://github.com/elevenlabs/ui),
 * `apps/www/registry/elevenlabs-ui/ui/bar-visualizer.tsx`.
 *
 * Copyright (c) 2025 Eleven Labs Inc. — MIT Licence.
 * Full licence text: LICENSE-NOTICE.md (Third-party components).
 *
 * Vendored rather than installed from their shadcn registry because that
 * registry targets Next.js app-router projects; this app is Vite + React 19.
 * The component itself needed no behavioural change — it is plain React and a
 * canvas, with no ElevenLabs SDK, no API key, and no cloud dependency, which
 * is exactly why it was chosen. Local modifications are marked `SONARI:`.
 */

// SONARI: dropped the Next.js `"use client"` directive — inert under Vite.
...
const generateConnectingSequenceBar = (columns: number): number[][] => {
  // SONARI: explicit element type — this repo's tsconfig is stricter than
  // the upstream registry's, and a bare `[]` infers `never[]` here.
  const seq: number[][] = [];
